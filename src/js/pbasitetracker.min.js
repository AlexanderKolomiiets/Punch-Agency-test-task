...
str=Object.keys(newobj).map(function(f){return newobj[f];}).join("&");}
...
obj[v]=params[v];});}
else if(Array.isArray(params)){params.forEach(function(ex){if(ex&&ex.name){if(ex.selector){var sel=ex.selector.split(/([\.@#][a-z:=\d]+)/gi).filter(function(i){return i;});;var tag="*";if(ex.selector.match(/^\w/)){tag=sel[0];sel.splice(0,1);}
...
var script=document.createElement('script');script.type='text/javascript';script.async=true;script.src=url;script.onload=script.onreadystatechange=function(){};var d=document.getElementsByTagName("script")[0];d.parentNode.insertBefore(script,d);}
...
setTimeout(function(){setupLinkClickEvent();},1000);}
...
function Init(){if(_needsInit&&!_waitingForCallback){_initTimeout+=_initTimeout;var hasDoneRequest=false;var user=getUser();if(_configs.useSiteCookie==true){if(user){setTimeout(function(){PbaSiteTracker.OnInitDone({success:true,uid:user});},0);}
...
if(hasDoneRequest){_waitingForCallback=true;_callbackTimeoutID=setTimeout(function(){_waitingForCallback=false;},10000);}}
if(_needsInit)
...
this.trackPage=function(extraArgs){this.track("_tp",extraArgs);};this.trackLinks=function(){if(_needsInit)
