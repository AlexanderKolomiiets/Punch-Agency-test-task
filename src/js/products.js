...
function create_ifrtrk() {
  ...
  extra = ["key=" + encodeURIComponent(weboidsync), "src=products.js", "ref=" + getEncodedHost() ];

  if (_ap_an.site){
    extra.push("site=" +encodeURIComponent( _ap_an.site ));
  }

  var params = extra.join('&');

  _send_after_cmp_check('iframe', 'https://cstatic.weborama.fr/iframe/sync.html?'+ params);
}

function retrieve_tpcs() {
	try {
